...
- `docker-compose.prod.yml` — `web` (Gunicorn) behind `nginx` (serves `/static/` and `/media/` directly, proxies the rest). Speaks plain HTTP — the hosting platform's Traefik performs TLS termination and the HTTP→HTTPS redirect. Named volumes for SQLite (`/app/db-data`) and media. Reads `env_file: .env` — **copy `.env.example` → `.env` (gitignored) and fill in `SECRET_KEY`, `DEBUG`, `ALLOWED_HOST`, `Q_COMING_CATEGORY_ID`**. `web` has a `/` healthcheck that `nginx` gates on (`service_healthy`).
...
- `nginx.conf` — matches `STATIC_URL` (`static/`) and `MEDIA_URL` (`/media/`); forwards Traefik's `X-Forwarded-Proto` so Django sees the real HTTPS scheme.
