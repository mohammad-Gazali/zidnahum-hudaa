...
- Generated files **are committed**. `frontend/src/app/shared/services/index.ts` is the hand-managed barrel: it re-exports `api/models`, every `api/<tag>`, and the hand-written services (auth, snackbar, confirmation, translate, memo, test, masjed, level, memorize-message-type, layout). Update it manually after a regenerate.
