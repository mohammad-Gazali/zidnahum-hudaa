...
- `docker-compose.prod.yml` — `web` (Gunicorn) behind a `traefik` reverse proxy. Traefik is configured entirely via `TRAEFIK_*` env vars; TLS with Let's Encrypt is always on: the app is served over HTTPS on :443 (with an HTTP→HTTPS redirect on :80). The certificate is requested for `ALLOWED_HOST` (must be the public domain), and the ACME account lives in `ACME_EMAIL`. Routing lives in `traefik/dynamic/app.yml` — a file-provider template that reads `ALLOWED_HOST` from the same `.env`, so the host is still an env edit with no rebuild. `/static/` is served by Whitenoise (mounted in settings.py's MIDDLEWARE) and `/media/` by Django's static.serve view (added unconditionally in urls.py), so Traefik only proxies. Named volumes for SQLite (`/app/db-data`), media, and ACME certs (`acme_data`). Reads `env_file: .env` — **copy `.env.example` → `.env` (gitignored) and fill in `SECRET_KEY`, `DEBUG`, `ALLOWED_HOST`, `ACME_EMAIL`, `Q_COMING_CATEGORY_ID`**. `web` has a `/` healthcheck that `traefik` gates on (`service_healthy`); traefik itself is healthchecked via its `/ping` on the internal `:8080` entrypoint.
