...
from students.constants import LAST_PART_MAP, LAST_PART_POINT_MAP, NEW, NON
...
def get_first_month_half_range():
  ...
  return [
    timezone.make_aware(
      timezone.datetime(year=now.year, month=now.month, day=1), now.tzinfo
    ),
    timezone.make_aware(
      timezone.datetime(year=now.year, month=now.month, day=16), now.tzinfo
    ),
  ]
...
def get_second_month_half_range():
  ...
  if now.day <= 15:
    return [
      timezone.make_aware(
        timezone.datetime(year=year_for_previous_month, month=previous_month, day=16),
        now.tzinfo,
      ),
      timezone.make_aware(
        timezone.datetime(year=now.year, month=now.month, day=1), now.tzinfo
  ...
  return [
    ...
    timezone.make_aware(
      timezone.datetime(year=year_for_next_month, month=next_month, day=1), now.tzinfo
    ),
  ]
...
def check_for_qtest(student, changes: List[int]) -> str | None:
  return _check_for_qtest_by_previously_qmemo(
    student, changes
  ) or _check_for_qtest_by_only_one_active_section(student, changes)
...
def _check_for_qtest_by_only_one_active_section(
  student, changes: List[int]
) -> str | None:
  COMPLETED = "completed"
  ACTIVE = "active"
  UNTOUCHED = "untouched"
  ...
  for item in changes:
    ...
    elif student_qtest_status[section_index] == ACTIVE and all(
      map(item_exists, student_qtest[8 * section_index : 8 * (section_index + 1)])
    ):
