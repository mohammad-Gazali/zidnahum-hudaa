...
from comings.serializers import (
  ComingCategorySerializer,
  ComingCreateSerializer,
  ComingListSerializer,
)
...
class ComingListCreateView(ListCreateAPIView):
  ...
  def get_queryset(self):
    ...
    return (
      Coming.objects.filter(master=self.request.user)
  ...
  def perform_create(self, serializer: ComingCreateSerializer):
    if Coming.objects.filter(
      **serializer.validated_data, registered_at__date=timezone.localtime().date()
    ):
      raise ValidationError(
        "you can't register a coming in for the same student and category in the same day"
      )
