...
param_name = OpenApiParameter(
  name="name",
  location=OpenApiParameter.QUERY,
  type=str,
  description="param for filtering student via his name or his id",
)
...
def create_serializer(
  ...
  `drf_spectacular` use the class name is the first method to distinguish between
  ...
  if exclude_fields:
    ...
    class Result(ModelSerializer):
      if (
        ...
        or ("id" in serializer_fields and not exclude_fields)
        or ("id" not in serializer_fields and exclude_fields)
      ):
        id = IntegerField()

      class Meta:
        ref_name = (
          f"{model_class.__name__}-{extra_ref}"
  ...
  class Result(BaseViewSet):
    ...
    def handle_exception(self, exc):
      if isinstance(exc, ProtectedError):
        ...
        return Response(
          {
            "detail": f"لا يمكن حذف العناصر لأنها مرتبطة بـ '{model.__class__._meta.verbose_name_plural}'"
          },
          HTTP_403_FORBIDDEN,
        )
    ...
    if model == Student:
      ...
      @extend_schema(parameters=[param_name])
    ...
    if listing_serializer is not None:
      ...
      @extend_schema(
        responses={
          HTTP_200_OK: details_serializer
          or create_serializer(model, fields, exclude_fields, "details")
        }
      )
    ...
    def get_serializer_class(self):
      if self.request.method == "GET":
        if self.kwargs.get("pk") is not None:
          return details_serializer or create_serializer(
            model, fields, exclude_fields, "details"
          )
        else:
          return listing_serializer or create_serializer(
            model, fields, exclude_fields, "list"
          )
      elif self.request.method == "POST":
        create_fields = fields.copy() if not isinstance(fields, str) else fields
        ...
        return creating_serializer or create_serializer(
          model, create_fields, create_exclude, "create"
        )
      else:
        update_fields = fields.copy() if not isinstance(fields, str) else fields
        ...
        return updating_serializer or create_serializer(
          model, update_fields, update_exclude, "update"
        )
